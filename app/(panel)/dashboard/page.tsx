...
async function getDashboardData(currentPage: number) {
	const [employeesResume, generalStats] = await Promise.all([
		employeesResumeDashboardService({
			params: {
				init: (currentPage - 1) * ITEMS_PER_PAGE_EMPLOYEES,
...
export default async function DashboardPage({
	...
	const { employeesResume, generalStats } = await getDashboardData(currentPage)
	...
	const isEmployeesResumeError = 'message' in employeesResume
	const isGeneralStatsError = 'message' in generalStats
	...
	return (
		<>
			...
			{isEmployeesResumeError ? (
				<ErrorState to="/dashboard">{employeesResume.message}</ErrorState>
			) : (
			...
			{isGeneralStatsError ? (
				<ErrorState to="/dashboard">{generalStats.message}</ErrorState>
			) : (
				<div className="bg-[#09090b] border border-white/5 rounded-3xl p-8">
					...
					<div className="flex items-end justify-between gap-4 h-48 px-2">
						{generalStats.charts.history.map((item) => {
							const maxHeight = 160
							const heightPixel =
								(item.count * maxHeight) / historyCountMaxInOneDay
